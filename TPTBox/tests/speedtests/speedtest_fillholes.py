if __name__ == "__main__":
    ...
    def np_nfill_holes_crop(nii, arr: np.ndarray):  # noqa: ARG001
        return np_fill_holes(arr)

    def np_fill_holes_extract2(nii, arr: np.ndarray):
        slice_wise_dim = None
        assert 2 <= arr.ndim <= 3
    ...
    def np_fill_holes_extract(nii, arr: np.ndarray):
        slice_wise_dim = None
        assert 2 <= arr.ndim <= 3
        assert arr.ndim == 3 or slice_wise_dim is None, "slice_wise_dim set but array is 3D"
        labels: list[int] = _to_labels(arr, None)
    ...
    speed_test(
        ...
        functions=[np_fill_holes_extract2, np_fill_holes_extract, np_nfill_holes_crop],
