...
def verify_missing_images(cache: DecisionCache, sub: str, subj_dict: dict) -> None:
    """For each expected base image absent from ``subj_dict``, ask the user whether it's really missing.

    If confirmed missing, drop the base and its dependent seg keys from
...
def verify_hardlink(sub: str, corrected_index_path: Path = _DEFAULT_CORRECTED_INDEX) -> None:
    """Run the loop for a single subject and verify what :func:`hard_link` would do.

    Applies corrections, traces what :func:`hard_link` *would* do, and checks
    every source exists and every target would land on the same filesystem as
    its source (so :func:`os.link` won't hit ``EXDEV``). Prints one line per
    file — no writes are performed.
    """
