...
from TPTBox.core.poi_fun.vertebra_pois_non_centroids import timing
from TPTBox.core.sitk_utils import nib_to_sitk, sitk_to_nib


@timing
def n4_bias_field_correction_nib2(
    ...
    input_ants = from_nibabel(image)
    if mask is not None:
        mask = from_nibabel(mask)  # type: ignore
    if weight_mask is not None:
        weight_mask = from_nibabel(weight_mask)  # type: ignore
...
def n4_bias_field_correction_sitk(
    ...
    shrink_factor: int = 4,
    convergence: dict[str, list[int] | float] | None = None,
    num_spline_param: int | None = None,
    return_bias_field: bool = False,
    verbose: bool = False,
    ...
    if weight_mask is not None:
        weight_mask = sitk.Cast(weight_mask, sitk.sitkUInt8)
        image = sitk.Mask(image, weight_mask)
    masked_image: sitk.Image = sitk.Mask(image, mask)
    mask_arr = sitk.GetArrayFromImage(masked_image)

    # Rescale intensities if required
    if rescale_intensities:
        original_min = floor(np.min(mask_arr))
        original_max = ceil(np.max(mask_arr))
    ...
    if num_spline_param is not None:
        corrector.SetNumberOfControlPoints(num_spline_param)
    if verbose:
