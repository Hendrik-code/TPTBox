...
def _read(filename, skip_voxels=False, verbos=True):
    ...
    for header_key in header:
        ...
        if header_key == "space":
            if header[header_key] == "left-posterior-superior":
                ...
                raise OSError("space field must be 'left-posterior-superior' or 'right-anterior-superior'")
            continue
            ...
            if header[header_key] == ["domain", "domain", "domain"]:
                ...
                raise OSError("kinds field must be 'domain domain domain' or 'list domain domain domain'")
            continue
            ...
            for parameter_str in parameters_str:
                ...
                if len(parameter_info) != 3:
                    raise OSError("Segmentation_ConversionParameters field value is invalid (each parameter must be defined by 3 strings)")
    ...
    segment_ids = set()
    for segments_field in segments_fields.values():
        if "ID" in segments_field:
            segment_ids.add(segments_field["ID"])
    ...
    for segment_index in sorted(segments_fields.keys()):
        ...
        if "Tags" in segment_fields:
            ...
            for tag_str in tags_str:
                tag_str = tag_str.strip()  # noqa: PLW2901
                if not tag_str:
                    continue
                ...
                if key == "TerminologyEntry":
                    segment_info["terminology"] = _terminology_entry_from_string(value)
...
def _write_segmentation(file, segmentation, compression_level=9, index_order=None):
    ...
    #   [ 0.        ,  0.        , -1.        ],
    #   [-1.29999542,  0.        ,  0.        ]]))
    assert ijkToLPS is not None, "no spical directions"
    ...
    if dims == 4:
        ...
        #   [ 0.        ,  0.        , -1.        ],
        #   [-1.29999542,  0.        ,  0.        ]]))
        space_directions = np.vstack(([np.nan, np.nan, np.nan], space_directions))
    elif dims != 3:
    ...
    for output_segment_index, segment in enumerate(segmentation["segments"]):
        ...
        for segment_key in segment:
            ...
            # ~SCT^85756007^Tissue~SCT^85756007^Tissue~^^~Anatomic codes - DICOM master list~^^~^^|
                ...
                terminology_str = _terminology_entry_to_string(segment[segment_key])
                ...
                for tag_key in tags:
                    output_tags.append(f"{tag_key}:{tags[tag_key]}")  # noqa: PERF401
                # Add tags later
                continue
        ...
        if "id" not in segment:
            ...
            new_segment_id = _generate_unique_segment_id(segment_ids)
        ...
        if "extent" not in segment:
            ...
            output_header[f"Segment{output_segment_index}_Extent"] = (
                f"0 {output_shape[0] - 1} 0 {output_shape[1] - 1} 0 {output_shape[2] - 1}"
            )
...
def _terminology_entry_from_string(terminology_str):
...
def _terminology_entry_to_string(terminology):
    ...
    typeModifier = terminology.get("typeModifier", ["", "", ""])
    ...
    if "anatomicContextName" in terminology:
        ...
        terminology_str += "~"
    anatomic_region = terminology.get("anatomicRegion", ["", "", ""])
    terminology_str += "~" + "^".join(anatomic_region)
    anatomic_region_modifier = terminology.get("anatomicRegionModifier", ["", "", ""])
...
def load_slicer_nrrd(filename, seg, skip_voxels=False, verbos=True) -> NII:
    ...
    if arr is None:
        raise ValueError("Segmentation file does not contain voxel data")
    remove_not_supported_values(nrrd_dict)

    # Affine: convert ijkToLPS to nifti RAS affine
    ijkToLPS = nrrd_dict.pop("ijkToLPS")

    # Convert LPS -> RAS for NIfTI
    # NIfTI uses RAS; LPS = diag(-1,-1,1) * RAS
    lps2ras = np.diag([-1, -1, 1, 1])
    affine = lps2ras @ ijkToLPS

    # Background value (usually 0)
    c_val = 0

    # Create NIfTI object
    nib_obj = nib.nifti1.Nifti1Image(arr, affine)
...
def save_slicer_nrrd(nii: NII, file: str | Path, make_parents=True, verbose: logging = True, compression_level=9, index_order=None):
    ...
    ijkToLPS = ras2lps @ affine
    info = getattr(nii, "info", {})
    # Build minimal segmentation dict
    info.pop("conversionParameters", None)

    segmentation = {
        "voxels": arr,
        "ijkToLPS": ijkToLPS,
        "encoding": info.get("encoding", "gzip"),
        **info,
    }
    if nii.seg:
    ...
    # Write using previously defined write_segmentation
    log.print(f"Saveing {file}", verbose=verbose, ltype=Log_Type.SAVE, end="\r")
    _write_segmentation(str(file), segmentation, compression_level=compression_level, index_order=index_order)
    log.print(f"Save {file}          ", verbose=verbose, ltype=Log_Type.SAVE)


if __name__ == "__main__":
    # {
    #    "voxels": np.ndarray or None,
    #    "ijkToLPS": 4x4 np.ndarray,
    #    "encoding": "gzip" or other,
    #    "containedRepresentationNames": [...],
    #    "conversionParameters": [...],
    #    "masterRepresentation": "Binary labelmap",
    #    "referenceImageExtentOffset": [...],
    #    "segments": [ {...}, {...}, ... ],
    #    ... (any additional non-segment NRRD header fields)
    # }
    import os
    ...
    os.makedirs(slicerio_data, exist_ok=True)
    base = "https://raw.githubusercontent.com/lassoan/slicerio/main/slicerio/data"

    files = {
        "CT": f"{base}/CTChest4.nrrd",
        "Seg": f"{base}/Segmentation.seg.nrrd",
    ...
    def test_file(path: str):
        ...
        arr = nii
        print("Shape:", arr, "Unique labels:", np.unique(arr))

        # Save to new file
        out_seg = path.replace(".seg.nrrd", ".roundtrip.seg.nrrd")
        ...
        # Compare
        same = np.array_equal(arr, arr2)
        print(f"Round trip match: {same}")
        # Path(out_seg).unlink(missing_ok=True)

    for file in os.listdir(slicerio_data):
        filepath = os.path.join(slicerio_data, file)
        if "roundtrip.seg" in filepath:
            continue
        test_file(filepath)
