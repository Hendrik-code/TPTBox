#!/usr/bin/env python
"""Compare two ``bench_speed.py`` JSON files and emit a markdown report.
...
threshold (see ``_compare_core`` for the exact gate).
    ...
    python benchmarks/nii_poi/compare.py baseline.json head.json --fail-on-regression-pct 50
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    report, regressed = compare(
        ...
        load_json(args.head),
        measurements_key="measurements_ms",
        unit="ms",
        threshold_pct=args.fail_on_regression_pct,
        alpha=args.alpha,
        title="Speed (wall time per call)",
    )
    print(report)
